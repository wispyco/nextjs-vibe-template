...
import { GenerationsProvider, useGenerations, DEFAULT_NUM_GENERATIONS, MIN_NUM_GENERATIONS, MAX_NUM_GENERATIONS } from '../GenerationsContext';
...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button data-testid="set-max" onClick={() => setNumGenerations(MAX_NUM_GENERATIONS)}>Set Max</button>
      <button data-testid="set-above-max" onClick={() => setNumGenerations(MAX_NUM_GENERATIONS + 10)}>Set Above Max</button>
...
describe('GenerationsContext', () => {
  ...
  it('should enforce maximum value when setting directly', () => {
    render(
      <GenerationsProvider>
        <TestComponent />
      </GenerationsProvider>
    ...
    fireEvent.click(screen.getByTestId('set-above-max'));

    // Should be at maximum
    expect(screen.getByTestId('num-generations').textContent).toBe(MAX_NUM_GENERATIONS.toString());
  });
